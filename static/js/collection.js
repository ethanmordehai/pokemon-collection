...
document.addEventListener("DOMContentLoaded", () => {
  ...
  bindStaticEvents();
  state.activeCollectionId = localStorage.getItem("pokecollect_collection_id") || "";
  loadCollection();
...
function buildManualCollection() {
  document.querySelector("#collectionView").innerHTML = `
    <section class="manual-hero">
      <div>
        <span class="eyebrow">Portefeuille scellé</span>
        <h1>PokéCollect</h1>
        <p>Plusieurs collections, prix d'achat, prix marché manuel, liens CardMarket et variation visible.</p>
      </div>
    ...
    <div class="manual-toolbar">
      <label class="select-wrap collection-switcher">
        <span>Collection</span>
        <select id="collectionSelect"></select>
      </label>
      <button class="primary-action" id="newCollectionBtn">+ Créer une nouvelle collection</button>
    ...
    <div class="modal-backdrop hidden" id="collectionModal" role="dialog" aria-modal="true">
      <form class="manual-modal collection-modal" id="collectionForm">
        <button class="icon-close" type="button" id="collectionCloseBtn" aria-label="Fermer">×</button>
        <h2>Nouvelle collection</h2>
        <label>Nom de la collection<input name="name" required placeholder="Collection collègue"></label>
        <p class="muted">La nouvelle collection démarre vide avec les mêmes catégories. Il pourra ensuite ajouter ses items dans les tableaux.</p>
        <button class="primary-action form-submit" type="submit">Créer la collection</button>
      </form>
    </div>
  `;
}
...
function bindStaticEvents() {
  document.querySelector("#collectionSelect").addEventListener("change", event => {
    state.activeCollectionId = event.target.value;
    localStorage.setItem("pokecollect_collection_id", state.activeCollectionId);
    loadCollection();
  });
  document.querySelector("#newCollectionBtn").addEventListener("click", openCollectionModal);
  ...
  document.querySelector("#manualExportBtn").addEventListener("click", () => {
    window.location.href = `/api/export/csv?collection_id=${encodeURIComponent(state.activeCollectionId)}`;
  });
  ...
  document.querySelector("#collectionCloseBtn").addEventListener("click", closeCollectionModal);
  document.querySelector("#collectionModal").addEventListener("click", event => {
    if (event.target.id === "collectionModal") closeCollectionModal();
  });
  document.querySelector("#collectionForm").addEventListener("submit", submitCollection);
}

async function loadCollection() {
  const payload = await api.collection(state.activeCollectionId);
  state.items = payload.collection.items;
  state.categories = payload.categories;
  state.collections = payload.collections || [];
  state.activeCollectionId = payload.active_collection_id || payload.collection.id;
  localStorage.setItem("pokecollect_collection_id", state.activeCollectionId);
...
function renderOptions() {
  const collectionOptions = state.collections.map(collection => {
    const selected = collection.id === state.activeCollectionId ? "selected" : "";
    return `<option value="${escapeHtml(collection.id)}" ${selected}>${escapeHtml(collection.name)}</option>`;
  }).join("");
  document.querySelector("#collectionSelect").innerHTML = collectionOptions;
  ...
  document.querySelector("#manualCategoryFilter").value = state.filter;
...
function renderCategoryTable(category) {
  ...
  const variation = totalBuy > 0 ? (pnl / totalBuy) * 100 : 0;
  return `
    <section class="manual-category" style="--category-color:${color}">
      <header class="manual-category-head">
        <h2>${escapeHtml(category)}</h2>
        <div>${items.length} lignes · Achat ${euro(totalBuy)} · Marché ${euro(totalMarket)} · ${variationBadge(variation)}</div>
      </header>
      <div class="manual-table-wrap">
        <table class="manual-table">
          ...
          <tbody>
            ${items.map(renderItemRow).join("") || `<tr><td colspan="9" class="empty-row">Aucun item dans cette catégorie.</td></tr>`}
            <tr class="manual-add-row">
              <td colspan="9"><button class="add-line-btn" data-add-category="${escapeHtml(category)}">+ Ajouter dans ${escapeHtml(category)}</button></td>
            </tr>
          </tbody>
        </table>
      </div>
...
function renderItemRow(item) {
  ...
  const market = Number(item.prix_marche || 0);
  const paid = Number(item.prix_achete || 0);
  const totalMarket = market * quantity;
  const totalBuy = paid * quantity;
  const pnl = totalMarket - totalBuy;
  const variation = paid > 0 ? ((market - paid) / paid) * 100 : null;
  ...
  return `
    <tr data-id="${escapeHtml(item.id)}">
      ...
      <td><span class="editable-cell" data-field="quantite" data-type="number">${quantity}</span></td>
      <td><span class="editable-cell" data-field="prix_achete" data-type="money">${euro(paid)}</span></td>
      <td><span class="editable-cell" data-field="prix_marche" data-type="money">${euro(market)}</span></td>
      <td>${euro(totalMarket)}</td>
      <td class="${pnl >= 0 ? "positive" : "negative"}">${euro(pnl)}</td>
      <td>${variationBadge(variation)}</td>
      <td>
        ${link}
...
function variationBadge(value) {
  if (value === null || Number.isNaN(Number(value))) return `<span class="manual-variation neutral">-</span>`;
  const numeric = Number(value);
  if (Math.abs(numeric) < 0.05) return `<span class="manual-variation neutral">0.0%</span>`;
  const cls = numeric >= 0 ? "up" : "down";
  const sign = numeric >= 0 ? "+" : "";
  const arrow = numeric >= 0 ? "▲" : "▼";
  return `<span class="manual-variation ${cls}">${arrow} ${sign}${numeric.toFixed(1)}%</span>`;
}
...
function bindTableEvents() {
  ...
  document.querySelectorAll("[data-delete]").forEach(button => {
    button.addEventListener("click", async event => {
      ...
      await api.deleteItem(id, state.activeCollectionId);
...
async function submitCollection(event) {
  ...
  if (!name) return;
  const payload = await api.createCollection(name);
  state.activeCollectionId = payload.active_collection_id;
  localStorage.setItem("pokecollect_collection_id", state.activeCollectionId);
...
async function submitManualItem(event) {
  ...
  await api.addItem(payload, state.activeCollectionId);
  closeManualModal();
  await loadCollection();
}
...
function startEdit(cell) {
  ...
  const save = async () => {
    ...
    await api.updateItem(id, { [field]: value, derniere_maj: new Date().toISOString().slice(0, 19) }, state.activeCollectionId);
    await loadCollection();
  };
